...
1. What is the purpose of using _sessions_? the purpose of using sessions is to have cookies for a client and that cookie saves info for that session like when we used sessions and cookies for authentication. 
1. What does bcrypt do to help us store passwords in a secure manner. bcrypt stores the password in an encrypted way instead of in plain text.
1. What does bcrypt do to slow down attackers? bcrypt will hash the password as many times as you want it to making that much harder for an attacker to decrypt it.
1. What are the three parts of the JSON Web Token? Header, payload, and signature
